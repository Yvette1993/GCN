...
def custom_loss(Y, A):
    ...
    # print(Gamma)
    loss = torch.sum(
        torch.mm(torch.div(Y.float(), Gamma.t()),
                 (1 - Y).t().float()) * A.float())
    return loss
...
def test_backward(Y, A):
    ...
    for i in range(gradient.shape[0]):
        for j in range(gradient.shape[1]):
            ...
            temp = A_i_alpha / torch.pow(
                Gamma[j], 2) * (Gamma[j] * (1 - 2 * Y[alpha, j]) - D[i] *
                                (Y[i, j] * (1 - Y[alpha, j]) + (1 - Y[i, j]) *
                                 (Y[alpha, j])))
            ...
            if (l2 != []):
                for val, mem in zip(l2_val, l2):
                    extra_gradient += (-D[i] * torch.sum(
                        Y[mem[0], j] *
                        (1 - Y[mem[1], j]) / torch.pow(Gamma[j], 2))) * val
...
def gene_graph_data(N, edges_num):
    '''
    Returns a test sparse SciPy adjecency matrix
    '''

    # 邻接矩阵的元素是 1
    data = np.ones(2 * edges_num)
    # 行列对应位置为一组邻接节点
    # temp1 = np.random.randint(0, N, node)
    # temp2 = np.random.randint(0, N, node)
    # row = np.append(temp1, temp2)
    # col = np.append(temp2, temp1)

    edge_set = set()
    while len(edge_set) < edges_num * 2:
    ...
    edge_list = [[i, j] for (i, j) in edge_set]
    # print(edge_list)

    temp_edge_np = np.array(edge_list).T
    row = temp_edge_np[0, :]
    col = temp_edge_np[1, :]
    fo = open("random_graph_data.txt", "w")
    for (i, j) in edge_set:

...
class DCGATEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.depth):
            ...
            blocks += [
                GATConv(in_channels_block,out_channels[i],heads=heads,dropout=dropout,concat=False)
            ]
    ...
    def forward(self, features, edge_indexes):
        ...
        for i in range(self.depth):
            ...
            features = self.layers[i](features, edge_indexes[hop])
            hop *= 3
...
class Netwrok(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # 表示模块
        self.encoder = DCGATEncoder(in_channels, out_channels, heads, dropout,
